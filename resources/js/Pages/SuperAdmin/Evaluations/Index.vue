...
function statusBadge(status) {
    if (status === 'finalized') return { label: 'Finalized', cls: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' };
    if (status === 'draft')     return { label: 'In Progress', cls: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' };
    return { label: 'Not Started', cls: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400' };
}
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Staff Performance Evaluations</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Weighted, role-aware appraisals scored out of 100%.</p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-4 shadow-sm">
                    ...
                    <div><p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.total }}</p><p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Staff</p></div>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-4 shadow-sm">
                    ...
                    <div><p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.finalized }}</p><p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Finalized</p></div>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-4 shadow-sm">
                    <div class="p-2 bg-amber-50 dark:bg-amber-900/30 rounded-lg"><ClockIcon class="h-5 w-5 text-amber-600 dark:text-amber-400" /></div>
                    <div><p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.pending }}</p><p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Pending</p></div>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-4 flex items-center gap-4 shadow-sm">
                    <div class="p-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg"><ChartBarIcon class="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
                    <div><p class="text-2xl font-bold" :class="scoreColor(stats.average_score)">{{ stats.average_score ?? '—' }}<span v-if="stats.average_score" class="text-base">%</span></p><p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Avg Score</p></div>
                </div>
            </div>
            ...
            <div class="relative max-w-md">
                ...
                <input v-model="search" type="text" placeholder="Search staff, position or role..." class="w-full pl-9 pr-3 py-2 text-sm bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
                <table class="min-w-full divide-y divide-gray-100 dark:divide-slate-700">
                    <thead class="bg-gray-50 dark:bg-slate-800/60">
                        <tr>
                            <th class="py-3 pl-5 pr-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Staff Member</th>
                            <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Position / Role</th>
                            <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Status</th>
                            <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Score</th>
                            <th class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 pr-5">Action</th>
                        </tr>
                    </thead>
                        ...
                        <tr v-for="s in filteredStaff" :key="s.id" class="hover:bg-gray-50/80 dark:hover:bg-slate-700/40 transition-colors cursor-pointer" @click="openStaff(s.id)">
                            ...
                            <td class="px-3 py-3.5">
                                <div v-if="s.overall_score !== null && s.overall_score !== undefined" class="flex items-baseline gap-1.5">
                                    ...
                                    <span class="text-[10px] font-medium text-gray-400">{{ s.grade }}</span>
                                </div>
                            ...
                            <td class="px-3 py-3.5 pr-5 text-right">
                                <span class="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                                    {{ s.status === 'finalized' ? 'View' : 'Evaluate' }}
