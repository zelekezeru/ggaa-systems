                        ...
                        <div v-if="task.document_path && task.document_path.length" class="mb-5 bg-indigo-50/50 p-3 rounded-lg border border-indigo-100">
                            ...
                            <div class="space-y-1.5">
                                <template v-for="(path, ix) in task.document_path" :key="ix">
                                    <a :href="'/storage/' + path" target="_blank" class="flex items-center gap-2 text-sm text-indigo-600 hover:text-indigo-800 transition-colors bg-white px-2.5 py-1.5 rounded-md shadow-sm border border-indigo-50">
